...
module.exports = {
  ...
  rooms: [
    // HOME LOCATIONS
    {
      ...
      description: "Your cozy bedroom. A desk with school supplies, a comfortable bed, and a window overlooking the neighborhood. You can go to the {{yellow}}kitchen{{color_reset}} or head to {{yellow}}school{{color_reset}}. Type {{cyan}}sleep{{color_reset}} to end the day.",
      exits: {
        kitchen: "kitchen",
        school: "school_gate"
      },
      ...
      onEnter: (state, engine) => {
        if (!flags.firstMorning) {
          ...
          flags.day = 1;
          flags.sakuraAffection = 0;
          flags.yukiAffection = 0;
          flags.hanaAffection = 0;
        }
      }
    },    
    {
  ...
  characters: [
...

    {
      ...
      aliases: ["head librarian", "old librarian"],
      genre: "female",
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        engine.printLine("{{cyan}}Type 'restart' to play again or 'quit' to exit.{{color_reset}}");
      }, 1000);
